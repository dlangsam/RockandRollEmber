...
module('Acceptance | Login', function(hooks) {
  ...
  test('Log in with valid credentials', async function(assert) {
    ...
    assert.dom('[data-test-rr=bands-empty-message]').hasText("Let's start by creating a band.", "A desriptive empty message is shown");
    assert.dom('[data-test-rr=user-email]').hasText('dave@tcv.com', "The logged in users's email is shown");
